...
| **预设提示词** | 8 个微/宏观维度预设 + 自定义，一键填充到输入框 |
...
│   ├── TreeNode.tsx        # 递归树节点
│   ├── RecursiveTree.tsx   # DndContext 包裹器
│   ├── GuideMap.tsx        # 导图视图
│   ├── TermLibrary.tsx     # 左侧术语列表
│   ├── ContextMenu.tsx     # 右键菜单工厂
│   ├── PlusMenu.tsx        # 加号下拉菜单
│   ├── SettingsPanel.tsx   # 设置弹窗（4 tab）
...
│   └── Onboarding.tsx
├── store/
│   ├── treeStore.ts        # 树 reducer（展开/收起/增删改）
│   ├── configStore.ts      # Zustand（LLM 配置 + 预设项）
│   └── footprintStore.ts   # 足迹 reducer
├── services/
...
│   └── termParser.ts       # [[术语]] 解析
├── hooks/
│   └── useConceptNode.ts   # IndexedDB + 缓存 hook
├── types/
│   └── index.ts            # 全部类型定义
└── lib/
